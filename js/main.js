...
import { VITESSE_MAX, REPERE_DECOLLAGE, TOUR_DUREE,
         ...
         PIECES, formatVitesse, formatAltitude, borne01 } from './model.js';
...
function image(ms) {
  try {
    ...
    // si une pièce est choisie (son anneau respire)
    const vif = sim.lecture || sim.moment || !sim.etat.auSol || sim.etat.v > 0.05 ||
      sim.cible > 0.001 || vuePieces.choisie !== null || !cache.premierDessin;
    if (vif) {
      cache.premierDessin = true;
      sim.etat = pas(sim.etat, sim.cible, dt);
      vueCote.dessine(sim.etat, sim.cible);
...
if (window.speechSynthesis && window.SpeechSynthesisUtterance) {
  ...
  $('btn-son').hidden = true; // pas de synthèse vocale : pas de version sonore
}

// ---- le bouton 🔇/🔊 : la voix des moments — clé de la FAMILLE, partagée
// entre tous les labos (même origine petit-labo.fr) ; l'ancienne clé de
// l'astronomie est lue une fois, en secours, pour ne pas perdre le réglage ----
...
function majBoutonSon() {
  btnSon.setAttribute('aria-pressed', sonActif ? 'true' : 'false');
}
majBoutonSon();
...
function raconteMoment() {
  if (conteur && sonActif && sim.moment) {
    conteur.parle(phrasesDe(pourLaVoix(sim.moment.moment.phrase), true));
  }
}
